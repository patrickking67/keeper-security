---
name: keeper-secrets
description: Retrieve, inject, and manage secrets from Keeper Vault via the KSM CLI (`ksm`). Use for accessing passwords, API keys, database credentials, and certificates stored in Keeper; injecting secrets into env vars at runtime (`ksm exec`); interpolating secrets into config files (`ksm interpolate`); creating, updating, or listing records; or syncing to AWS/Azure secret stores. Triggers include 'ksm', 'keeper secrets', 'keeper vault', 'keeper notation', 'keeper://', and credential injection for CI/CD, Docker, or Kubernetes pipelines. Always prefer this over hardcoding credentials. For user/team/role/SSO administration or KSM Application creation, use the keeper-admin skill instead.
---
