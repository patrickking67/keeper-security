...
@mcp.tool(
    description=(
        "Escape hatch: run any Keeper Commander CLI command non-interactively and return "
        "its raw output. Covers the full command set beyond the curated tools: record-add/"
        "record-update (always use password=$GEN, never plaintext), enterprise-team/-role/"
        "-node, msp-add/-update/-billing-report, compliance-report, share-report, "
        "user-report, trash, sync-down, msp-down, help <command>, and more. Add "
        "--format=json where supported. Mutating/destructive commands (rm, msp-remove, "
        "enterprise-user --delete) execute exactly as given - confirm with the user first. "
        "Interactive commands (shell, login, connect) and logout are refused."
    ),
